...
try {
  ...
  await page.locator(".metric", { hasText: "Version cargada" }).filter({ hasText: "knowledge-v28" }).first().waitFor();
  await page.locator(".metric", { hasText: "Validacion" }).filter({ hasText: "381 pendientes" }).first().waitFor();
  ...
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v28" }).locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "26" }).waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v28" }).getByText("+1 fuentes").waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v28" }).getByText("+5 nodos").waitFor();
  ...
  await explorationPanel.locator(".metric", { hasText: "Nodos" }).filter({ hasText: "129" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Evidencias" }).filter({ hasText: "127" }).waitFor();
  ...
  await queryPanel.locator(".metric", { hasText: "Version recuperada" }).filter({ hasText: "knowledge-v28" }).waitFor();
  ...
  await page
    .getByText("0 fichas, 0 claims y 0 evidencias en version knowledge-v28.")
    .waitFor();
  ...
  const historyLimitResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("version") === "knowledge-v28" &&
      url.searchParams.get("limit") === "50"
    );
  });
  await page.getByLabel("Limite historial").selectOption("50");
  await historyLimitResponse;
  await page.getByText("knowledge-v28 -> consulta").first().waitFor();
  ...
  const filteredAuditResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("entity_id") === "knowledge-v28"
    );
  });
  await page.getByLabel("Filtro auditoria").selectOption("Consultas de conocimiento");
  await filteredAuditResponse;
  await page
    .getByText("knowledge-v28 -> consulta · 0 fichas · 0 claims · 0 evidencias")
    .first()
    .waitFor();
  ...
  await page.getByText("Version navegada: knowledge-v28").waitFor();
} finally {
  await browser.close();
}
