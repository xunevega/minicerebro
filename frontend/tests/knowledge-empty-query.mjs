...
try {
  ...
  await page.locator(".metric", { hasText: "Version cargada" }).filter({ hasText: "knowledge-v30" }).first().waitFor();
  await page.locator(".metric", { hasText: "Validacion" }).filter({ hasText: "411 pendientes" }).first().waitFor();
  ...
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v30" }).locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "26" }).waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v30" }).getByText("0 fuentes").waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v30" }).getByText("+5 nodos").waitFor();
  ...
  await explorationPanel.locator(".metric", { hasText: "Nodos" }).filter({ hasText: "139" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Evidencias" }).filter({ hasText: "137" }).waitFor();
  ...
  await queryPanel.locator(".metric", { hasText: "Version recuperada" }).filter({ hasText: "knowledge-v30" }).waitFor();
  ...
  await page
    .getByText("0 fichas, 0 claims y 0 evidencias en version knowledge-v30.")
    .waitFor();
  ...
  const historyLimitResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("version") === "knowledge-v30" &&
      url.searchParams.get("limit") === "50"
    );
  });
  await page.getByLabel("Limite historial").selectOption("50");
  await historyLimitResponse;
  await page.getByText("knowledge-v30 -> consulta").first().waitFor();
  ...
  const filteredAuditResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("entity_id") === "knowledge-v30"
    );
  });
  await page.getByLabel("Filtro auditoria").selectOption("Consultas de conocimiento");
  await filteredAuditResponse;
  await page
    .getByText("knowledge-v30 -> consulta · 0 fichas · 0 claims · 0 evidencias")
    .first()
    .waitFor();
  ...
  await page.getByText("Version navegada: knowledge-v30").waitFor();
} finally {
  await browser.close();
}
