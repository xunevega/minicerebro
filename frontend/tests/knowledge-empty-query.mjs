...
try {
  ...
  await page.locator(".metric", { hasText: "Version cargada" }).filter({ hasText: "knowledge-v15" }).first().waitFor();
  await page.locator(".metric", { hasText: "Validacion" }).filter({ hasText: "186 pendientes" }).first().waitFor();
  ...
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v15" }).locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "13" }).waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v15" }).getByText("+1 fuentes").waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v15" }).getByText("+5 nodos").waitFor();
  ...
  await sourceExplorerPanel.getByText("Publicadas · 13").waitFor();
  ...
  await sourceExplorerPanel.getByText("Registradas pendientes · 10").waitFor();
  ...
  await ingestionPanel.locator(".metric", { hasText: "Publicadas" }).filter({ hasText: "13" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "Ingeridas" }).filter({ hasText: "13" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "No ingeridas" }).filter({ hasText: "10" }).waitFor();
  await ingestionPanel.getByText("publicada: 13").waitFor();
  await ingestionPanel.getByText("con edicion: 10").waitFor();
  ...
  await explorationPanel.locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "13" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Nodos" }).filter({ hasText: "64" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Evidencias" }).filter({ hasText: "62" }).waitFor();
  ...
  await queryPanel.locator(".metric", { hasText: "Version recuperada" }).filter({ hasText: "knowledge-v15" }).waitFor();
  ...
  await page
    .getByText("0 fichas, 0 claims y 0 evidencias en version knowledge-v15.")
    .waitFor();
  ...
  const historyLimitResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("version") === "knowledge-v15" &&
      url.searchParams.get("limit") === "50"
    );
  });
  await page.getByLabel("Limite historial").selectOption("50");
  await historyLimitResponse;
  await page.getByText("knowledge-v15 -> consulta").first().waitFor();
  ...
  const filteredAuditResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("entity_id") === "knowledge-v15"
    );
  });
  await page.getByLabel("Filtro auditoria").selectOption("Consultas de conocimiento");
  await filteredAuditResponse;
  await page
    .getByText("knowledge-v15 -> consulta · 0 fichas · 0 claims · 0 evidencias")
    .first()
    .waitFor();
  ...
  await page.getByText("Version navegada: knowledge-v15").waitFor();
} finally {
  await browser.close();
}
