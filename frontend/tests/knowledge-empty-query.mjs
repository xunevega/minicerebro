...
try {
  ...
  await libraryPanel.locator(".statusPill", { hasText: "213 fichas" }).waitFor({ timeout: 90000 });
  ...
  await gymPanel.locator(".metric", { hasText: "Fichas" }).filter({ hasText: "213" }).waitFor({
    timeout: 90000,
  });
  ...
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v46" }).locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "26" }).waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v46" }).getByText("0 fuentes").waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v46" }).getByText("+5 nodos").waitFor();
  ...
  await pipelinePanel.locator(".pipelineCard", { hasText: "Glosario de terminos gramaticales" }).getByText("Extraccion").waitFor();
  ...
  await explorationPanel.locator(".metric", { hasText: "Nodos" }).filter({ hasText: "215" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Evidencias" }).filter({ hasText: "213" }).waitFor();
  ...
  await queryPanel.locator(".metric", { hasText: "Base recuperada" }).filter({ hasText: "knowledge-v46" }).waitFor();
  ...
  await historyItem.getByRole("button", { name: "Ver detalle" }).click();
