...
try {
  ...
  await page.locator(".metric", { hasText: "Version cargada" }).filter({ hasText: "knowledge-v14" }).first().waitFor();
  await page.locator(".metric", { hasText: "Validacion" }).filter({ hasText: "171 pendientes" }).first().waitFor();
  ...
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v14" }).locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "12" }).waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v14" }).getByText("+1 fuentes").waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v14" }).getByText("+5 nodos").waitFor();
  ...
  await sourceExplorerPanel.getByText("Publicadas · 12").waitFor();
  ...
  await sourceExplorerPanel.getByText("Registradas pendientes · 11").waitFor();
  ...
  await sourceExplorerPanel.locator(".sourceMiniCard", { hasText: "Retorica" }).getByText("available · publicada").waitFor();
  const ingestionPanel = page.locator(".proposalBox", { hasText: "Registro frente a ingestion" });
  await ingestionPanel.locator(".metric", { hasText: "Publicadas" }).filter({ hasText: "12" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "Ingeridas" }).filter({ hasText: "12" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "No ingeridas" }).filter({ hasText: "11" }).waitFor();
  await ingestionPanel.getByText("publicada: 12").waitFor();
  await ingestionPanel.getByText("con edicion: 11").waitFor();
  ...
  await ingestionPanel.locator(".ingestionItem", { hasText: "Retorica" }).getByText("publicada").waitFor();
  ...
  await explorationPanel.locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "12" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Nodos" }).filter({ hasText: "59" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Evidencias" }).filter({ hasText: "57" }).waitFor();
  ...
  await queryPanel.locator(".metric", { hasText: "Version recuperada" }).filter({ hasText: "knowledge-v14" }).waitFor();
  ...
  await page
    .getByText("0 fichas, 0 claims y 0 evidencias en version knowledge-v14.")
    .waitFor();
  ...
  const historyLimitResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("version") === "knowledge-v14" &&
      url.searchParams.get("limit") === "50"
    );
  });
  await page.getByLabel("Limite historial").selectOption("50");
  await historyLimitResponse;
  await page.getByText("knowledge-v14 -> consulta").first().waitFor();
  ...
  const filteredAuditResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("entity_id") === "knowledge-v14"
    );
  });
  await page.getByLabel("Filtro auditoria").selectOption("Consultas de conocimiento");
  await filteredAuditResponse;
  await page
    .getByText("knowledge-v14 -> consulta · 0 fichas · 0 claims · 0 evidencias")
    .first()
    .waitFor();
  ...
  await page.getByText("Version navegada: knowledge-v14").waitFor();
} finally {
  await browser.close();
}
