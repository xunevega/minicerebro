...
try {
  ...
  await page.locator(".metric", { hasText: "Version cargada" }).filter({ hasText: "knowledge-v5" }).first().waitFor();
  await page.locator(".metric", { hasText: "Validacion" }).filter({ hasText: "15 pendientes" }).first().waitFor();
  ...
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v5" }).locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "5" }).waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v5" }).getByText("+1 fuentes").waitFor();
  ...
  await sourceExplorerPanel.getByText("Publicadas · 5").waitFor();
  ...
  await sourceExplorerPanel.getByText("Registradas pendientes · 18").waitFor();
  ...
  await ingestionPanel.locator(".metric", { hasText: "Publicadas" }).filter({ hasText: "5" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "Ingeridas" }).filter({ hasText: "5" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "No ingeridas" }).filter({ hasText: "18" }).waitFor();
  await ingestionPanel.getByText("publicada: 5").waitFor();
  await ingestionPanel.getByText("con edicion: 18").waitFor();
  ...
  await explorationPanel.locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "5" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Nodos" }).filter({ hasText: "7" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Evidencias" }).filter({ hasText: "5" }).waitFor();
  ...
  await queryPanel.locator(".metric", { hasText: "Version recuperada" }).filter({ hasText: "knowledge-v5" }).waitFor();
  ...
  await page
    .getByText("0 fichas, 0 claims y 0 evidencias en version knowledge-v5.")
    .waitFor();
  ...
  const historyLimitResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("version") === "knowledge-v5" &&
      url.searchParams.get("limit") === "50"
    );
  });
  await page.getByLabel("Limite historial").selectOption("50");
  await historyLimitResponse;
  await page.getByText("knowledge-v5 -> consulta").first().waitFor();
  ...
  const filteredAuditResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("entity_id") === "knowledge-v5"
    );
  });
  await page.getByLabel("Filtro auditoria").selectOption("Consultas de conocimiento");
  await filteredAuditResponse;
  await page
    .getByText("knowledge-v5 -> consulta · 0 fichas · 0 claims · 0 evidencias")
    .first()
    .waitFor();
  ...
  await page.getByText("Version navegada: knowledge-v5").waitFor();
} finally {
  await browser.close();
}
