...
try {
  ...
  await page.locator(".metric", { hasText: "Version cargada" }).filter({ hasText: "knowledge-v18" }).first().waitFor();
  await page.locator(".metric", { hasText: "Validacion" }).filter({ hasText: "231 pendientes" }).first().waitFor();
  ...
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v18" }).locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "16" }).waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v18" }).getByText("+1 fuentes").waitFor();
  await versionPanel.locator(".versionItem", { hasText: "knowledge-v18" }).getByText("+5 nodos").waitFor();
  ...
  await sourceExplorerPanel.getByText("Publicadas · 16").waitFor();
  ...
  await sourceExplorerPanel.getByText("Registradas pendientes · 7").waitFor();
  ...
  await ingestionPanel.locator(".metric", { hasText: "Publicadas" }).filter({ hasText: "16" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "Ingeridas" }).filter({ hasText: "16" }).waitFor();
  await ingestionPanel.locator(".metric", { hasText: "No ingeridas" }).filter({ hasText: "7" }).waitFor();
  await ingestionPanel.getByText("publicada: 16").waitFor();
  await ingestionPanel.getByText("con edicion: 7").waitFor();
  ...
  await explorationPanel.locator(".metric", { hasText: "Fuentes" }).filter({ hasText: "16" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Nodos" }).filter({ hasText: "79" }).waitFor();
  await explorationPanel.locator(".metric", { hasText: "Evidencias" }).filter({ hasText: "77" }).waitFor();
  ...
  await queryPanel.locator(".metric", { hasText: "Version recuperada" }).filter({ hasText: "knowledge-v18" }).waitFor();
  ...
  await page
    .getByText("0 fichas, 0 claims y 0 evidencias en version knowledge-v18.")
    .waitFor();
  ...
  const historyLimitResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("version") === "knowledge-v18" &&
      url.searchParams.get("limit") === "50"
    );
  });
  await page.getByLabel("Limite historial").selectOption("50");
  await historyLimitResponse;
  await page.getByText("knowledge-v18 -> consulta").first().waitFor();
  ...
  const filteredAuditResponse = page.waitForResponse((response) => {
    ...
    return (
      ...
      url.searchParams.get("entity_id") === "knowledge-v18"
    );
  });
  await page.getByLabel("Filtro auditoria").selectOption("Consultas de conocimiento");
  await filteredAuditResponse;
  await page
    .getByText("knowledge-v18 -> consulta · 0 fichas · 0 claims · 0 evidencias")
    .first()
    .waitFor();
  ...
  await page.getByText("Version navegada: knowledge-v18").waitFor();
} finally {
  await browser.close();
}
