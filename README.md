...
- Ficha de usuario en `GET/POST /profiles/{profile_id}/knowledge-cards/{card_id}` y `GET /profiles/{profile_id}/knowledge-cards`: guarda feedback, score personal, elementos que mantiene y cambios que pide para una ficha/version de conocimiento sin modificar la `KnowledgeCard` publicada ni el snapshot estable.
