...
- Catalogo cerrado de 23 fuentes V1 registrado en `/knowledge/sources`; `POST /knowledge/sources` permite registrar una fuente persistente sin crear edicion ni publicar directamente; `POST /knowledge/sources/{source_id}/editions`, `GET /knowledge/sources/{source_id}/editions` y `GET /knowledge/editions/{edition_id}` registran y leen ediciones bibliograficas sin iniciar indice, lote de ingestion ni publicacion; cada fuente expone sus ediciones, estados de adquisicion/validacion y sistema de localizadores.
...
- Publicacion V1 en `/knowledge/publication` y `/knowledge/publication/readiness`: publicar significa convertir una `knowledge_version` completa en conocimiento estable recuperable; no hay publicaciones parciales y una version publicada solo puede deprecarse o archivarse mediante una version posterior.
...
- Recuperacion V1 en `/knowledge/query`: usa la interpretacion de consulta, recupera fichas como unidad principal, conserva claims/evidencias/fuentes, limita relaciones, desglosa ranking y devuelve traza reproducible sin mezclar perfil y conocimiento.
