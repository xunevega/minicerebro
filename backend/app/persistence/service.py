...
def persistence_domains() -> list[PersistenceDomain]:
    return [
        _domain("knowledge", "knowledge_sources + knowledge_nodes + knowledge_cards", "persisted"),
        ...
        _domain("versions", "knowledge_versions", "persisted"),
...
def _domain(identifier: str, storage: str, status: str) -> PersistenceDomain:
    return PersistenceDomain(
        ...
        storage=storage,
        status=status,
        separated_from_knowledge=identifier == "knowledge" or not storage.startswith("knowledge_"),
    )
